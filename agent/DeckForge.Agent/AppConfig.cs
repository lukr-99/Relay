...
/// <summary>
/// Runtime configuration + on-disk locations. Everything lives under %AppData%\DeckForge\ so it
...
public sealed class AppConfig
{
    ...
    public static AppConfig Load()
    {
        var dir = Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "DeckForge");
        Directory.CreateDirectory(dir);

        var statePath = Path.Combine(dir, "deckforge.state.json");
        var state = AgentState.LoadOrCreate(statePath);

        return new AppConfig
        {
            DataDir = dir,
            LogPath = Path.Combine(dir, "logs", "deckforge.log"),
